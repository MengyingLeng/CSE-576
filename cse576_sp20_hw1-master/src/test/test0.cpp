...
void test_rgb_to_hsv()
  ...
  //cout<<"Test rgb to hsv"<<endl;
  TEST(same_image(im, hsv));
  //im.save_image("output/dog_rgb_to_hsv");
  }
...
void test_hsv_to_rgb()
  ...
  hsv_to_rgb(im);
  TEST(same_image(im, c));
      im.save_image("output/dog_hsv_to_rgb");
  }
...
//              {
//                  float error = im(q1,q2,ch)-c(q1,q2,ch);
//                  //float error = (im(q1,q2,c)-c(q1,q2,c))/c(q1,q2,c);
//                  maxError = max(error,maxError);
//                  minError = min(error,minError);
//
//              }
//      cout<<maxError<<" "<<minError;
  TEST(same_image(im, c));
