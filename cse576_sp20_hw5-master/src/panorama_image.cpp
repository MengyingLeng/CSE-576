...
//  NOT_IMPLEMENTED();
    vector<int> a_b = match_descriptors_a2b(a,b);
    vector<int> b_a = match_descriptors_a2b(b,a);
    int n = a.size();
    for(int i = 0; i < n; i++){
        if(i == b_a[a_b[i]]){
            float distance = l1_distance(a[i].data,b[a_b[i]].data);

            const Descriptor* aa = &a[i];
            const Descriptor* bb = &b[a_b[i]];
//
//            printf("match a %f %f   ",aa->p.x, aa->p.y);
//            printf("match b %f %f \n",bb->p.x, bb->p.y);
...
//        Match c = m[j];
//        printf("m_calculate %f, %f, %f, %f \n",c.a->p.x,c.a->p.y, c.b->p.x, c.b->p.y);
//    }
//    printf("\n");
    return m;
}
...
//  NOT_IMPLEMENTED();
    ...
    mp(1,0) = p.y;
    mp(2,0) = 1;
//  printf("H col %d  row %d",H.cols, H.rows);
//    printf("mp col %d  row %d",mp.cols, mp.rows);
    Matrix res = H * mp;
...
//      print_matrix(H);
        ...
        vector<Match> inliers = model_inliers(H,  m, thresh);
//        printf("inliers.size() %d",inliers.size());
...
//    printf("dx %d, dy %d  topleft y %f",dx,dy,topleft.y);
