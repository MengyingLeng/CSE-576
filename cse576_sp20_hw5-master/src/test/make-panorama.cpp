...
void do_rainier(void)
  ...
  image_map im;
  load_images(im,indir,outdir,6,0,720);  // im, dir, numpics (0..numpics-1),  PROJ_METHOD,  FOCAL_LEN
...
void do_helens(void)
{
    ...
    th.push_back(thread([&](){create_panorama(im,"1-0","1","0" ,2,0,0.05,11,7,5,150000,100,0.5);}));
    th.push_back(thread([&](){create_panorama(im,"3-2","3","2" ,2,0,0.05,11,7,5,150000,100,0.5);}));
    th.push_back(thread([&](){create_panorama(im, "2-1", "2", "1", 2, 0, 0.05, 5, 7, 5, 50000, 100, 0.5); }));
    th.push_back(thread([&](){create_panorama(im,"4-3","4","3" ,2,0,0.05,15,5,3,150000,100,0.5);}));
    th.push_back(thread([&]() {create_panorama(im, "5-4", "5", "4", 3, 0, 0.035, 17, 5, 5, 150000, 100, 0.5); }));
    for(auto&e1:th)e1.join();th.clear();
    printf("wwwww");

    create_panorama(im, "3--1", "3-2", "2-1", 2, 0, 0.05, 11, 7, 5, 150000, 100, 0.5);
    create_panorama(im,"3--0", "3--1",  "1-0" ,2,0,0.05,11,7,4,30000,100,0.5);
    create_panorama(im, "4--0", "4-3", "3--0", 2, 0, 0.05, 5, 7, 4, 30000, 100, 0.5);
    create_panorama(im,"all" , "5-4", "4--0" ,2,0,0.05,11,7,5,50000,100,0.5);
...
void do_sun(void)
  ...
  create_panorama(im,"all" , "0-1", "2--4" ,2,0,0.05,14,7,5,50000,100,0.5);
